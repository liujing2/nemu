...
static void virt_pci_init(VirtMachineState *vms)
{
    ...
    vms->acpi_conf.pci_host = g_new(PCIHostState*, vms->acpi_conf.segment_nr);

    for (i = 0; i < vms->acpi_conf.segment_nr; i++) {
        pci_memory[i] = g_new(MemoryRegion, 1);
        snprintf(name, sizeof(name), "%x.pci", i);
        memory_region_init(pci_memory[i], NULL, name, UINT64_MAX);
        vms->acpi_conf.pci_host[i] = pci_lite_init(get_system_memory(),
                                                   get_system_io(),
                                                   pci_memory[i], i);
        vms->pci_bus[i] = vms->acpi_conf.pci_host[i]->bus;
    }
}
...
static void virt_machine_state_init(MachineState *machine)
{
    ...
    fw_cfg_add_i64(fw_cfg, FW_CFG_PCIEXBAR_BASE, PCI_HOST_PCIEXBAR_BASE);
    fw_cfg_add_i16(fw_cfg, FW_CFG_PCI_SEGMENT, vms->acpi_conf.segment_nr);
    rom_set_fw(fw_cfg);
...
{
    VirtMachineState *vms = VIRT_MACHINE(obj);
    uint16_t value = vms->acpi_conf.segment_nr;
                                        ...
                                        Error **errp)
{
    ...
    vms->acpi_conf.segment_nr = value;
out:
